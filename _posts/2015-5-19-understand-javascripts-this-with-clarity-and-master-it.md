...
`this` 这个变量 **永远** 指向 **一个** 对象，并且拥有这个对象的值。虽然 `this` 可以在全局作用域中出现，但它通常还是会在函数体内或对象的方法内。有一点要注意的是，当我们使用严格模式（struct mode）的时候，`this` 在全局函数中和匿名函数中的值是未定义的（undefined），不指向任何一个对象。
